...
export default async function ComparePage({ searchParams }: { searchParams: Promise<{ a?: string; b?: string; c?: string }> }) {
  ...
  const first = query.a ?? demoShoes[0]?.slug ?? "";
  const second = query.b ?? demoShoes.find((shoe) => shoe.slug !== first)?.slug ?? "";
  const third = query.c ?? "";
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-14 lg:px-8">
      ...
      <div className="mt-10"><CompareBuilder shoes={demoShoes} initial={[first, second, third]} /></div>
    </div>
  );
}
