...
export function CatalogBrowser({ shoes }: { shoes: DemoShoe[] }) {
  ...
  return (
    <div>
      <div className="grid gap-0 border-y border-black/10 md:grid-cols-[1.6fr_1fr_1fr_auto_auto]">
        ...
        <label className="flex items-center gap-2 border-b border-black/10 py-4 text-sm md:border-b-0 md:border-l md:pl-5"><input type="checkbox" checked={localOnly} onChange={(e) => setLocalOnly(e.target.checked)} /> ID brands</label>
      ...
      <div className="mt-5 flex items-center justify-between text-sm text-black/50"><span>{filtered.length} models shown</span><span>{shoes.filter((shoe) => shoe.currentPrice).length} current price checks</span></div>
